...
void HistoryData::take_sample(uint32_t tm, float data) {
...
void HistoryData::take_sample(float data) {
  uint32_t tm = millis();
  this->take_sample(tm, data);
}

void HistoryData::take_sample(std::string json_str) {
  ESP_LOGI(TAG, "Init trace for sensor %s", json_str.c_str());
  uint32_t base = millis();
  json::parse_json(json_str, [&, this](JsonObject root) { 
    uint32_t base = millis();
    for (JsonPair kv : root) {
      uint32_t hour_millis = base + atoi(kv.key().c_str()) * 3600 * 1000;
      ESP_LOGI("add", "k %s", kv.key().c_str());
      ESP_LOGI("add", "h %d", hour_millis);
      ESP_LOGI("add", "v %f", kv.value().as<float>());
      this->take_sample(hour_millis, kv.value().as<float>());
    }
    return true;
  });
}
...
void GraphTrace::init(Graph *g) {
  ...
  sensor_->add_on_state_callback([this](std::string state) { this->data_.take_sample(state); });
...
void GraphLegend::init(Graph *g) {
  ...
  for (auto *trace : g->traces_) {
    ...
    if (this->values_ != VALUE_POSITION_TYPE_NONE) {
      ...
      ss << std::fixed << std::setprecision(2) << trace->sensor_->get_state();
